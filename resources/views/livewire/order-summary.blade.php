...
>
            ...
            <li class="py-4 flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <template x-if="item.variant_name">
                        <p class="text-xs text-muted-text mt-0.5" x-text="item.variant_name"></p>
                    </template>
                    <p class="text-xs text-muted-text mt-0.5" x-text="\"Số lượng: \" + item.quantity"></p>
                </div>
    ...
    <div class="border-t border-border-subtle pt-5 space-y-3">
        <div class="flex justify-between text-sm">
            ...
            <span x-text="fmt(breakdown.subtotal)">{{ number_format($breakdown['subtotal'] ?? 0, 0, ',', '.') }}₫</span>
        </div>

        <template x-if="showShipping">
            <div class="flex justify-between text-sm">
                <span class="text-muted-text">Phí vận chuyển</span>
                <span x-text="breakdown.final_shipping_fee <= 0 ? \"Miễn phí\" : fmt(breakdown.final_shipping_fee)">@if(($breakdown['final_shipping_fee'] ?? 0) <= 0) Miễn phí @else {{ number_format($breakdown['final_shipping_fee'], 0, ',', '.') }}₫ @endif</span>
            </div>
        </template>

        @if($showPromotions && count($breakdown['applied_rules'] ?? []) > 0)
            <div class="py-2 border-t border-dashed border-border-subtle space-y-2">
                <span class="text-[10px] font-semibold text-muted-text uppercase tracking-wider block">Ưu đãi áp dụng</span>
                <div class="space-y-1.5">
                    @foreach($breakdown['applied_rules'] as $rule)
                        <div class="flex justify-between items-center text-xs text-emerald-700 bg-emerald-50/80 px-2.5 py-1.5 rounded border border-emerald-200/60">
                            <div class="flex items-center gap-1.5 truncate max-w-[70%]">
                                ...
                                <span class="truncate font-medium">{{ $rule['rule_name'] }}</span>
                                @if($rule['is_coupon'])
                                    <span class="text-[9px] bg-emerald-200 text-emerald-800 px-1 rounded uppercase font-bold">Mã coupon</span>
                                @endif
                            </div>
                            <span class="font-bold shrink-0">-{{ number_format($rule['discount_amount'], 0, ',', '.') }}₫</span>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <template x-if="showPromotions && breakdown.totalDiscount > 0">
            <div class="flex justify-between text-sm text-emerald-700 font-medium">
                ...
                <span x-text="\"-\" + fmt(breakdown.total_discount)">-{{ number_format($breakdown['total_discount'] ?? 0, 0, ',', '.') }}₫</span>
            </div>
        </template>
        ...
        <div class="flex justify-between pt-4 border-t border-border-subtle items-baseline">
            ...
            <span class="text-xl font-bold text-badge-hot" x-text="fmt(breakdown.final_total)">{{ number_format($breakdown['final_total'] ?? ($breakdown['subtotal'] ?? 0), 0, ',', '.') }}₫</span>
        </div>
    </div>
