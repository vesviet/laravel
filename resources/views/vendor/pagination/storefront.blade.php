@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
        ...
        @if ($paginator->onFirstPage())
            <span class="text-sm text-muted-text cursor-not-allowed">Trang trước</span>
        @else
        ...
        <div class="hidden sm:flex items-center gap-4">
            @foreach ($elements as $element)
                @if (is_string($element))
                    <span class="text-sm text-muted-text">{{ $element }}</span>
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <a href="{{ $url }}" class="text-sm text-muted-text hover:text-[#1a1a1a] transition-colors pb-0.5">{{ $page }}</a>
                        @endif
                    @endforeach
                @endif
            @endforeach
        ...
        @else
            <span class="text-sm text-muted-text cursor-not-allowed">Trang sau</span>
        @endif
    </nav>
@endif
