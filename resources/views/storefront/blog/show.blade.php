...
<div class="bg-surface-bg py-8 md:py-12 border-b border-border-subtle">
    <div class="section-wrapper">
        {{-- Breadcrumbs --}}
        <nav class="flex flex-wrap items-center gap-2 text-xs text-muted-text font-light mb-6 uppercase tracking-wider" aria-label="Breadcrumb">
            <a href="{{ route('home') }}" class="hover:text-primary-dark transition-colors">Trang Chủ</a>
            <span>/</span>
            <a href="{{ route('blog.index') }}" class="hover:text-primary-dark transition-colors">Blog</a>
            @if($post->category)
                <span>/</span>
                <a href="{{ route('blog.index', ['category' => $post->category->slug]) }}" class="hover:text-primary-dark transition-colors">
                    {{ $post->category->name }}
                </a>
            @endif
            <span>/</span>
            <span class="text-primary-dark font-normal truncate max-w-xs md:max-w-md">{{ $post->title }}</span>
        </nav>

        <div class="max-w-4xl">
            @if($post->category)
                <a href="{{ route('blog.index', ['category' => $post->category->slug]) }}"
                   class="inline-block px-3 py-1 bg-white text-primary-dark text-[10px] font-semibold tracking-[0.2em] uppercase border border-border-subtle mb-4 hover:bg-primary-dark hover:text-white transition-colors">
                    {{ $post->category->name }}
                </a>
            @endif

            <h1 class="text-3xl md:text-5xl font-light text-primary-dark leading-tight mb-6">
                {{ $post->title }}
            </h1>

            {{-- Meta Row & Social Share --}}
            <div class="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-border-subtle">
                <div class="flex items-center gap-3 text-xs text-muted-text font-light">
                    <div class="w-8 h-8 rounded-full bg-primary-dark text-white flex items-center justify-center font-medium text-xs">
                        {{ strtoupper(substr($post->author?->name ?? 'M', 0, 1)) }}
                    </div>
                    <div>
                        <p class="font-medium text-primary-dark">{{ $post->author?->name ?? 'MYSHOP Editorial' }}</p>
                ...
                <div class="flex items-center gap-3">
                    <span class="text-[10px] font-medium tracking-[0.15em] uppercase text-muted-text">Chia Sẻ:</span>
                    {{-- Facebook --}}
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
                       target="_blank" rel="noopener noreferrer"
                       class="w-8 h-8 rounded-full bg-white border border-border-subtle flex items-center justify-center text-muted-text hover:text-primary-dark hover:border-primary-dark transition-colors"
...
@if($post->banner_image_url || $post->featured_image_url)
    <div class="section-wrapper py-6 md:py-10">
        <div class="relative aspect-[21/9] md:aspect-[21/9] overflow-hidden bg-[#E8E4DF] border border-border-subtle">
...
<div class="section-wrapper py-10 md:py-16">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <main class="lg:col-span-8 flex flex-col">
            ...
            @if(!empty($toc))
                <div class="lg:hidden mb-10 p-6 bg-white border border-border-subtle" x-data="{ open: true }">
                    <div class="flex items-center justify-between cursor-pointer" @click="open = !open">
                        <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark flex items-center gap-2">
                            <span>Mục Lục Bài Viết</span>
                        </h3>
                        <span x-text="open ? '−' : '+'" class="text-base text-muted-text"></span>
                    </div>
                    <nav x-show="open" class="mt-4 pt-4 border-t border-border-subtle flex flex-col gap-2.5" aria-label="Mục lục bài viết di động">
                        @foreach($toc as $item)
                            <a href="#{{ $item['id'] }}"
                               class="text-xs text-muted-text hover:text-primary-dark hover:underline transition-colors {{ $item['level'] === 3 ? 'pl-4' : 'font-medium text-primary-dark' }}">
                                {{ $item['title'] }}
                            </a>
                        @endforeach
                    </nav>
            ...
            {{-- Excerpt Highlight --}}
            @if($post->excerpt)
                <div class="p-6 md:p-8 bg-white border-l-4 border-primary-dark mb-10 text-base md:text-lg font-light text-primary-dark leading-relaxed italic">
                    {{ $post->excerpt }}
                </div>
            @endif
            ...
            <article class="prose prose-lg max-w-none text-primary-dark font-light leading-relaxed
                            ...
                            prose-img:rounded-none prose-img:border prose-img:border-border-subtle prose-img:my-8
                            prose-blockquote:border-l-2 prose-blockquote:border-primary-dark prose-blockquote:italic prose-blockquote:font-light prose-blockquote:pl-6
            ...
            @if(!empty($post->faq_schema) && is_array($post->faq_schema) && count($post->faq_schema) > 0)
                <section class="mt-14 pt-10 border-t border-border-subtle" aria-label="Câu hỏi thường gặp">
                    ...
                    <div class="space-y-4">
                        @foreach($post->faq_schema as $faq)
                            <div class="bg-white border border-border-subtle p-5" x-data="{ open: false }">
                                <button type="button"
                                        @click="open = !open"
            ...
            <div class="mt-14 pt-8 border-t border-border-subtle flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button"
                            @click="navigator.clipboard.writeText(window.location.href); $dispatch('toast', { message: 'Đã sao chép liên kết vào bộ nhớ tạm!', type: 'success' })"
                            class="px-4 py-2 bg-white border border-border-subtle text-xs uppercase tracking-wider text-primary-dark hover:bg-primary-dark hover:text-white transition-colors">
                        Sao Chép Link
                    </button>
                </div>
            </div>
            ...
            @if($post->products->isNotEmpty())
                <section class="mt-16 pt-12 border-t-2 border-primary-dark" aria-label="Sản phẩm trong bài viết">
                    <div class="mb-8">
                        <p class="text-[10px] font-semibold tracking-[0.25em] uppercase text-muted-text mb-1">Contextual Commerce</p>
                        <h2 class="text-2xl md:text-3xl font-light text-primary-dark">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        @foreach($post->products as $product)
                            <article class="bg-white border border-border-subtle flex flex-col group hover:shadow-md transition-shadow duration-300" aria-label="{{ $product->name }}">
                                ...
                                <div class="p-4 flex flex-col flex-1 justify-between text-center">
                                    <div>
                                        @if($product->category)
                                            <p class="text-[9px] font-semibold text-muted-text uppercase tracking-[0.2em] mb-1">
                                                {{ $product->category->name }}
                                            </p>
                                        @endif
                                        <h3 class="text-xs font-normal text-primary-dark line-clamp-1 mb-2">
                                            <a href="{{ route('products.show', $product->slug) }}" class="hover:text-[#666666] transition-colors">
                                                {{ $product->name }}
                                            </a>
                                        </h3>
                                        <p class="text-sm font-medium text-primary-dark mb-4">
                                            {{ number_format($product->price, 0, ',', '.') }}₫
                                        </p>
                                    </div>
                                    <a href="{{ route('products.show', $product->slug) }}"
                                       class="w-full py-2.5 bg-primary-dark text-white text-[10px] font-medium tracking-[0.2em] uppercase hover:bg-black transition-colors block">
                                        Xem Chi Tiết
                                    </a>
                                </div>
                            </article>
        ...
        <aside class="lg:col-span-4 space-y-8 sticky top-24">
            ...
            @if(!empty($toc))
                <div class="hidden lg:block bg-white border border-border-subtle p-6 shadow-sm">
                    <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark pb-3 border-b border-border-subtle mb-4 flex items-center gap-2">
                        <span>Mục Lục Bài Viết</span>
                    </h3>
                    <nav class="flex flex-col gap-2.5 max-h-[360px] overflow-y-auto pr-2" aria-label="Mục lục bài viết">
                        @foreach($toc as $item)
                            <a href="#{{ $item['id'] }}"
                               class="text-xs text-muted-text hover:text-primary-dark hover:underline transition-colors leading-relaxed {{ $item['level'] === 3 ? 'pl-4' : 'font-medium text-primary-dark' }}">
                                {{ $item['title'] }}
                            </a>
                        @endforeach
                    </nav>
            ...
            @if($post->products->isNotEmpty())
                ...
                <div class="bg-white border border-border-subtle p-6 shadow-sm">
                    <div class="flex items-center justify-between pb-3 border-b border-border-subtle mb-4">
                        <span class="text-[9px] font-semibold tracking-[0.2em] uppercase text-muted-text">Gợi Ý Trong Bài</span>
                        <span class="px-2 py-0.5 bg-primary-dark text-white text-[9px] font-semibold tracking-widest uppercase">
                            Featured
                        </span>
                    </div>
                        ...
                        <a href="{{ route('products.show', $primaryProduct->slug) }}">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center bg-[#E8E4DF]">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 text-muted-text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                                    </svg>
                                </div>
                            @endif
                    ...
                    <h4 class="text-sm font-normal text-primary-dark mb-1">
                    ...
                    <a href="{{ route('products.show', $primaryProduct->slug) }}"
                       class="w-full py-3 bg-primary-dark text-white text-[10px] font-medium tracking-[0.2em] uppercase hover:bg-black transition-colors block text-center">
                        Xem Sản Phẩm &rarr;
                    </a>
                </div>
            @endif
            ...
            @if($relatedPosts->isNotEmpty())
                <div class="bg-white border border-border-subtle p-6 shadow-sm">
                    <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark pb-3 border-b border-border-subtle mb-4">
                        ...
                        @foreach($relatedPosts as $related)
                            <article class="flex items-start gap-3 group" aria-label="{{ $related->title }}">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-xs font-normal text-primary-dark line-clamp-2 leading-snug group-hover:text-[#666666] transition-colors mb-1">
                                        <a href="{{ route('blog.show', $related->slug) }}">
                                            {{ $related->title }}
                                        </a>
                                    </h4>
                                    <p class="text-[10px] text-muted-text font-light">
