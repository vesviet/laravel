    ...
    <div class="section-wrapper">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-2xl font-light text-primary-dark tracking-wide uppercase">Chương Trình Giới Thiệu & Điểm Thưởng</h1>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                <div class="bg-white border border-border-subtle p-4 text-center">
                    <p class="text-[10px] uppercase tracking-wider text-muted-text font-medium mb-1">Đã Giới Thiệu</p>
                    <p class="text-2xl font-bold text-primary-dark">{{ $stats["total_referrals"] }}</p>
                </div>
                <div class="bg-white border border-border-subtle p-4 text-center">
                    <p class="text-[10px] uppercase tracking-wider text-muted-text font-medium mb-1">Hoàn Tất Đơn</p>
                ...
                <div class="bg-white border border-border-subtle p-4 text-center">
                    <p class="text-[10px] uppercase tracking-wider text-muted-text font-medium mb-1">Mã Của Bạn</p>
                    <p class="text-xl font-bold font-mono text-primary-dark">{{ $stats["referral_code"] }}</p>
                </div>
            </div>

            {{-- Referral Code & Share --}}
            <div class="bg-white border border-border-subtle p-6 shadow-sm mb-8">
                <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark mb-4 pb-3 border-b border-border-subtle">Mã Giới Thiệu Của Bạn</h3>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-xs text-muted-text font-light">Mã giới thiệu</p>
                            <p class="font-mono text-xl font-bold text-primary-dark">{{ $stats["referral_code"] }}</p>
                        </div>
                    </div>
                ...
                <div class="bg-[#F9F9F9] border border-border-subtle p-4 rounded-lg">
                    <p class="text-xs text-muted-text font-light mb-2">Link giới thiệu của bạn:</p>
            ...
            {{-- How It Works --}}
            <div class="bg-white border border-border-subtle p-6 shadow-sm mb-8">
                <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark mb-4">Cách Thức Hoạt Động</h3>
                <div class="grid sm:grid-cols-3 gap-6 text-center">
                    <div class="p-4">
                        ...
                        <h4 class="font-medium text-primary-dark mb-1">Chia Sẻ Mã</h4>
                        <p class="text-xs text-muted-text">Gửi mã hoặc link cho bạn bè</p>
                    </div>
                    <div class="p-4">
                        ...
                        <h4 class="font-medium text-primary-dark mb-1">Bạn Bè Mua Hàng</h4>
                    ...
                    <div class="p-4">
                        ...
                        <h4 class="font-medium text-primary-dark mb-1">Nhận Thưởng</h4>
            ...
            {{-- Loyalty Points --}}
            <div class="bg-white border border-border-subtle p-6 shadow-sm mb-8">
                <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark mb-4">Điểm Thưởng & Đổi Voucher</h3>
                <div class="grid sm:grid-cols-2 gap-6">
                    <div>
                        <h4 class="font-medium text-primary-dark mb-2">Tích Điểm</h4>
                        <ul class="text-sm text-muted-text space-y-1">
                    ...
                    <div>
                        <h4 class="font-medium text-primary-dark mb-2">Đổi Voucher</h4>
                        <ul class="text-sm text-muted-text space-y-1">
            ...
            <div class="bg-white border border-border-subtle p-6 shadow-sm">
                <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark mb-4 pb-3 border-b border-border-subtle">Lịch Sử Giới Thiệu</h3>

                @if($referrals->count() > 0)
                    <div class="space-y-4">
                        @foreach($referrals as $referral)
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-[#F9F9F9] border border-border-subtle rounded-lg">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 bg-surface-bg border border-border-subtle rounded-full flex items-center justify-center shrink-0">
                                        <span class="text-sm font-medium text-muted-text">{{ Str::upper($referral->name[0]) }}</span>
                                    </div>
                                    <div>
                                        <p class="font-medium text-primary-dark">{{ $referral->name }}</p>
                                        <p class="text-xs text-muted-text">{{ $referral->email }}</p>
                                    </div>
                                </div>

                                <div class="flex items-center gap-4 text-sm">
                                    <span class="{{ $referral->orders_count > 0 ? "text-emerald-700" : "text-muted-text" }} font-medium">
                                        {{ $referral->orders_count > 0 ? "Đã đặt hàng (" . $referral->orders_count . " đơn)" : "Chưa đặt hàng" }}
                                    </span>
                                    <span class="text-xs text-muted-text">{{ $referral->created_at->format("d/m/Y") }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-sm font-medium text-primary-dark mb-2">Chưa có giới thiệu nào</p>
                        <p class="text-xs text-muted-text font-light mb-6">Hãy chia sẻ mã giới thiệu để bắt đầu kiếm thưởng.</p>
                        <a href="{{ route("account.referrals.share") }}" class="inline-block px-6 py-2.5 bg-primary-dark text-white hover:bg-black text-xs font-semibold uppercase tracking-wider transition-colors">
                            Chia Sẻ Ngay
                        </a>
                    </div>
                @endif
