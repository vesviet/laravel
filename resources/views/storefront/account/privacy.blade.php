    ...
    <div class="section-wrapper">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-2xl font-light text-primary-dark tracking-wide uppercase">Quyền Riêng Tư & Dữ Liệu</h1>
            ...
            <div class="bg-white border border-border-subtle p-6 shadow-sm mb-8">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div>
                        <h3 class="text-lg font-medium text-primary-dark mb-1">Xuất Dữ Liệu Cá Nhân</h3>
                        <p class="text-sm text-muted-text">Tải xuống tất cả dữ liệu chúng tôi lưu về bạn (JSON)</p>
                    </div>
            ...
            {{-- Privacy Settings --}}
            <div class="bg-white border border-border-subtle p-6 shadow-sm mb-8">
                <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark mb-4 pb-3 border-b border-border-subtle">Cài Đặt Quyền Riêng Tư</h3>
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            @foreach($customer->privacy_consent as $key => $value)
                                <div class="bg-[#F9F9F9] border border-border-subtle p-4 rounded-lg">
                                    <p class="text-xs uppercase tracking-wider text-muted-text font-medium mb-1">{{ Str::ucfirst(str_replace("_", " ", $key)) }}</p>
                                    <p class="text-sm font-medium text-primary-dark">{{ is_bool($value) ? ($value ? "Đã đồng ý" : "Chưa đồng ý") : $value }}</p>
                                </div>
                            @endforeach
                        </div>
                    @else
                        <p class="text-sm text-muted-text">Chưa có bản ghi đồng ý quyền riêng tư.</p>
                    @endif
                </div>
            </div>
            ...
            {{-- Audit Log --}}
            <div class="bg-white border border-border-subtle p-6 shadow-sm mb-8">
                <h3 class="text-xs font-semibold tracking-[0.2em] uppercase text-primary-dark mb-4 pb-3 border-b border-border-subtle">Lịch Sử Hoạt Động (Audit Log)</h3>

                @if($auditLogs->count() > 0)
                    <div class="space-y-3 max-h-96 overflow-y-auto">
                        @foreach($auditLogs as $log)
                            <div class="flex flex-col sm:flex-row gap-3 p-3 bg-[#F9F9F9] border border-border-subtle rounded-lg">
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    @if($log->description)
                                        <span class="text-xs text-muted-text">{{ $log->description }}</span>
                                    @endif
                                </div>
                                <div class="text-xs text-muted-text font-light">
                                ...
                                @if($log->old_values || $log->new_values)
                                    <details class="text-xs text-muted-text">
                                        <summary class="cursor-pointer text-blue-600 hover:underline">Xem chi tiết thay đổi</summary>
                                        <pre class="mt-2 p-2 bg-white border border-border-subtle rounded text-[10px] overflow-x-auto">{{ json_encode(array_filter(["old" => $log->old_values, "new" => $log->new_values]), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                    </details>
                                @endif
                            </div>
                        @endforeach
                ...
                @else
                    <div class="text-center py-8">
                        <p class="text-sm text-muted-text">Chưa có hoạt động nào được ghi nhận.</p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-rose-50 border border-rose-200 p-6 shadow-sm rounded-lg">
                ...
                <form method="POST" action="{{ route("account.privacy.delete") }}" onsubmit="return confirm(\"Bạn có chắc chắn muốn xóa tài khoản? Hành động này KHÔNG THỂ HOÀN TÁC.\")" class="space-y-4">
                    ...
                    @error("confirmation")
                        <span class="text-badge-hot text-xs block">{{ $message }}</span>
                    @enderror
