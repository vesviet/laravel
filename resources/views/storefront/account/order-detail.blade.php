...
<div class="py-10 md:py-14 bg-[#FAFAFA]">
    <div class="section-wrapper">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="mb-6">
                <a href="{{ route('account.orders') }}" class="text-xs uppercase tracking-wider font-semibold text-primary-dark hover:text-badge-hot inline-flex items-center gap-1.5 link-underline">
            ...
            <div class="bg-white border border-border-subtle shadow-sm">
                ...
                {{-- Order Header --}}
                <div class="p-6 bg-surface-light border-b border-border-subtle flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <span class="text-[10px] uppercase font-bold tracking-widest text-muted-text">Mã Đơn Hàng</span>
                        <h1 class="text-lg sm:text-xl font-bold text-primary-dark">{{ $order->order_number }}</h1>
                ...
                <div class="p-6 border-b border-border-subtle grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs">
                    <div>
                        <h3 class="text-[10px] font-bold tracking-[0.2em] uppercase text-muted-text mb-2">Thông Tin Nhận Hàng</h3>
                        <p class="font-semibold text-primary-dark text-sm">{{ $order->customer_name }}</p>
                    ...
                    <div>
                        <h3 class="text-[10px] font-bold tracking-[0.2em] uppercase text-muted-text mb-2">Phương Thức Thanh Toán</h3>
                        ...
                        @if($order->notes)
                            <div class="mt-3 p-2.5 bg-[#F9F9F9] border border-border-subtle text-[11px] text-[#555555]">
                ...
                {{-- Items List --}}
                <div class="p-6">
                    <h3 class="text-[10px] font-bold tracking-[0.2em] uppercase text-muted-text mb-4">Danh Sách Sản Phẩm</h3>
                    <ul class="divide-y divide-[#EBEBEB]">
                        @foreach($order->items as $item)
                            <li class="py-4 flex items-center justify-between gap-4">
                                <div class="flex items-center gap-3.5 min-w-0">
                                    <div class="w-14 h-14 bg-surface-light border border-border-subtle shrink-0 overflow-hidden flex items-center justify-center">
                    ...
                    <div class="mt-6 pt-4 border-t border-[#EBEBEB] space-y-2 text-xs">
                        ...
                        <div class="flex justify-between text-sm font-bold text-primary-dark pt-3 border-t border-[#EBEBEB]">
                            <span>Tổng thanh toán</span>
                            <span class="text-base text-badge-hot">{{ $order->formatted_total_amount }}</span>
                        </div>
                    </div>
                </div>

                {{-- Footer Actions --}}
                <div class="p-6 bg-[#F9F9F9] border-t border-border-subtle flex flex-wrap items-center justify-between gap-4">
                    <form method="POST" action="{{ route('account.orders.reorder', $order->order_number) }}">
                        @csrf
                        <button type="submit" class="px-5 py-2.5 bg-primary-dark text-white hover:bg-black text-xs font-semibold uppercase tracking-wider transition-colors cursor-pointer">
                    ...
                    @if($order->is_cancellable)
                        <form method="POST" action="{{ route('account.orders.cancel', $order->order_number) }}" onsubmit="return confirm('Bạn có chắc chắn muốn hủy đơn hàng này?')">
                            @csrf
                            <button type="submit" class="px-5 py-2.5 border border-badge-hot text-badge-hot hover:bg-badge-hot hover:text-white text-xs font-semibold uppercase tracking-wider transition-colors cursor-pointer">
                                Hủy Đơn Hàng
                            </button>
                        </form>
                    @endif
